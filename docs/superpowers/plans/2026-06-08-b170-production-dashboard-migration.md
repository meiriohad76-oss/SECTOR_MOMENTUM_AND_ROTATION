...
- [ ] Replace Streamlit custom click/tooltip bridges with native React interactions.
  ...
  - [x] Native React/SVG value-specific RRG and flow-river tooltips exist with keyboard-selectable RRG points.
...
- [x] Add Playwright screenshot QA against the design handoff PNG/HTML references.
  ...
  - [x] Latest profile-C evidence targets `http://127.0.0.1:3100/?presentation=c`; required text/nonblank checks pass for C1/C2/C3 with current similarity 0.8210 overview, 0.8378 deep dive, and 0.8525 rotation after moving the C top bar, weather strip, right rail, and C1 heatmap closer to the handoff bar-logo, control glyphs, outer weather band, padded rail cards, full-width legend strip, compact state pills, shared pillar-bar visual domain, signed score/momentum formatting, transition-journal-backed state changes, data-derived live-universe overflow groups, live-row density, shortened data-derived weather BLUF, palette, padded panel, header, legend, and row-grid treatment while keeping provenance as heatmap metadata, while remaining below a pixel-parity release gate. The C1 capture height is now 3058px versus the 2911px reference, down from the prior 3905px live-universe capture while keeping all current run-journal rows available in collapsed per-class overflow groups.
