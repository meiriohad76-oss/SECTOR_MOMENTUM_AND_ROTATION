...
- [x] Add Playwright screenshot QA against the design handoff PNG/HTML references.
  ...
  - [x] Latest profile-C evidence targets `http://127.0.0.1:3100/?presentation=c`; required text/nonblank checks pass for C1/C2/C3 with current similarity 0.7928 overview, 0.8378 deep dive, and 0.8525 rotation after moving the C top bar, weather strip, right rail, and C1 heatmap closer to the handoff bar-logo, control glyphs, outer weather band, padded rail cards, palette, padded panel, header, legend, and row-grid treatment while keeping provenance as heatmap metadata, while remaining below a pixel-parity release gate.
