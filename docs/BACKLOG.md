...
**Files:** `src/api_contract.py`, `src/api_status.py`, `src/api_data_health.py`, `src/api_dashboard_snapshot.py`, `src/api_backtest_artifacts.py`, `src/api_ticker_chart.py`, `src/api_portfolio.py`, `src/api_saved_portfolios.py`, `src/api_refresh.py`, `src/api_refresh_runner.py`, `src/api_server.py`, `scripts/capture_next_handoff_qa.py`, `scripts/check_b170_retirement_readiness.py`, `scripts/serve_next_qa_api.py`, `web/package.json`, `web/package-lock.json`, `web/app/page.tsx`, `web/app/dashboard-screens-client.tsx`, `web/app/chart-primitives.tsx`, `web/app/globals.css`, `web/lib/api.ts`, `systemd/sector-api.service`, `systemd/sector-next.service`, `tests/test_api_contract.py`, `tests/test_api_status.py`, `tests/test_api_data_health.py`, `tests/test_api_dashboard_snapshot.py`, `tests/test_api_backtest_artifacts.py`, `tests/test_api_ticker_chart.py`, `tests/test_api_portfolio.py`, `tests/test_api_saved_portfolios.py`, `tests/test_api_refresh.py`, `tests/test_api_refresh_runner.py`, `tests/test_api_server_refresh_endpoint.py`, `tests/test_api_server_static.py`, `tests/test_next_handoff_qa_script.py`, `tests/test_next_qa_api_script.py`, `tests/test_next_shell_static.py`, `tests/test_b170_next_deploy_static.py`, `tests/test_b170_retirement_readiness_script.py`, `requirements.txt`, `docs/browser-qa/next-handoff/latest`, `docs/DEPLOY_RASPBERRY_PI.md`, `docs/DEPLOY_CLOUDFLARE_TUNNEL.md`, `docs/superpowers/plans/2026-06-08-b170-production-dashboard-migration.md`.
**Behavior:** adds a pure JSON status contract for dashboard health lanes and an optional FastAPI app factory with `/api/v1/health`, `/api/v1/status`, `GET /api/v1/data-health`, `GET /api/v1/provider-health`, `GET /api/v1/dashboard-snapshot`, `GET /api/v1/backtest-artifacts`, `GET /api/v1/ticker-chart`, `POST /api/v1/portfolio/analyze`, `GET /api/v1/portfolios`, `POST /api/v1/portfolios`, `DELETE /api/v1/portfolios?name=...`, `POST /api/v1/refresh`, `GET /api/v1/refresh/{job_id}`, and `GET /api/v1/refresh/{job_id}/events`. The default API status provider reads persisted state-machine storage, run-journal metadata, and Massive provider-snapshot coverage without importing Streamlit, fetching providers, recomputing indicators, or mutating live scoring state. The data/provider-health API additionally exposes provider-flow readiness and provider-flow cache coverage from persisted/configured state only, so future clients can show provider health without live provider calls. The backtest artifact API reports existing manual report/equity/state artifact presence, timestamps, hashes, metadata verification state, report text, and equity rows without running backtests, calibration, provider fetches, or research jobs; the Next shell consumes it in a collapsed Backtest Lab panel with artifact statuses, report preview, and an API-row equity preview. The ticker chart API reads cached OHLCV only, computes weekly close, 30-week moving average, CMF(21), OBV, OBV slope, benchmark-relative RS ratio, 12-week momentum, and 52-week momentum rows, and the React C2 deep dive fetches the selected ticker chart asynchronously while falling back to saved gate evidence if cached chart data is unavailable. The portfolio analysis API accepts a single ticker, JSON holdings, CSV text, or base64 CSV/XLSX content and maps holdings onto the latest persisted dashboard snapshot without provider fetches, scoring recompute, broker calls, portfolio persistence, or state writes; saved-portfolio routes explicitly reuse `data/saved_inputs.json` for named portfolio list/save/delete. The Next shell now includes an API-backed portfolio analyzer for ticker, pasted CSV, and uploaded CSV/XLS/XLSX inputs, with exposure summaries, missing ticker diagnostics, click-through rows into the dashboard focus, and explicit save/load/delete controls for named portfolios. Refresh jobs and progress events are persisted in local SQLite so a future React/Next.js frontend can create refresh requests and reconnect to progress state while the current dashboard keeps running. The refresh endpoint is queue-only by default; when a caller explicitly sends `run_now: true`, `src/api_refresh_runner.py` executes the existing headless Massive/FRED/dashboard refresh sequence and records phase progress. The first Next.js shell under `web/` consumes `/api/v1/health`, `/api/v1/data-health`, and `/api/v1/dashboard-snapshot`, renders persisted/provider health lanes plus A/B/C overview, deep-dive, and rotation sections from the latest run-journal snapshot, keeps provider calls behind the API, and now has native React screen selection, sortable overview lists, ticker focus selection, click-through cards/rows, rotation quadrant filtering, API-fed SVG primitives for pillar-stack rows, ticker waterfall, RRG, momentum bars, a relative-strength/momentum deep-dive panel, and a data-derived flow river, a light `?presentation=c` Display C candidate layout, plus a repeatable Playwright screenshot QA harness against the C1/C2/C3 handoff PNGs.
**Activation:** install `requirements.txt`, build the Next app with `npm --prefix web run build`, then optionally install the candidate `sector-api.service` and `sector-next.service` templates; no Pi service switch is made by this ticket.
**Migration plan:** keep Streamlit as the production frontend until API-backed React screens reach parity. The candidate API/Next services and Cloudflare route plan are documented for a separate `next-sentimentdashboard.ahaddashboards.uk` hostname, while `sentimentdashboard.ahaddashboards.uk` stays routed to Streamlit. Streamlit route remains production until feature parity, data parity, visual parity, and rollback gates are accepted.
**Evidence:** Next handoff browser QA screenshots and similarity reports are stored in `docs/browser-qa/next-handoff/latest`; the latest `?presentation=c` run has required text/nonblank passes for C1/C2/C3 with current similarity 0.8210 for overview, 0.8378 for deep dive, and 0.8579 for rotation after the C top bar, weather strip, right rail, C1 heatmap, and C3 rotation screen moved closer to the handoff bar-logo, control glyphs, outer weather band, padded rail cards, full-width legend strip, compact state pills, shared pillar-bar visual domain, signed score/momentum formatting, transition-journal-backed state changes, data-derived live-universe overflow groups, live-row density, shortened data-derived weather BLUF, palette, padded panel, compact rotation header, live row-count chart metadata, header/legend/row-grid treatment, gradient pressure lanes, and live outflow/inflow pressure captioning. The C1 capture height is now 3058px versus the 2911px reference, down from the prior 3905px live-universe capture while keeping all current run-journal rows available in collapsed per-class overflow groups. The latest `?presentation=a` terminal run has required text/nonblank passes for A1/A2/A3 with current similarity 0.8704 for overview, 0.8736 for deep dive, and 0.8939 for rotation. The latest `?presentation=b` editorial run has required text/nonblank passes for B1/B2/B3 with current similarity 0.8427 for overview, 0.8134 for deep dive, and 0.8454 for rotation. The overview now uses cumulative stacked pillar bars and data-backed positions/brief rails instead of handoff fixture holdings, with the QA helper preserving the current real row universe while trimming duplicate transport payload sections. The C1 overview also removes the visible run-journal/debug rail card while preserving provenance as heatmap metadata. The deep dive now has C2/A2/B2-style waterfall, terminal, and editorial variants, weighted pillar-detail cards with evidence tags, run-journal-backed gate/evidence panels, numbered live-data article paragraphs, compact editorial gate tables, and cached ticker-chart sidebar panels so the first-pass visual structure remains closer to the handoff while retaining extra real-data panels on demand. The rotation screen now keeps flow-river/terminal/editorial rotation compositions backed by live snapshot rows for flow-pressure sorting, row-specific flow explanations, and a light editorial RRG/leaderboard treatment for B3.
**Residual risk:** this is a foundation slice only. The API runner can execute the real headless refresh when explicitly requested, and the initial Next shell now has lockfile/audit/build evidence plus journal-backed A/B/C screen foundations, native React interactions, API-fed chart primitives, async cached ticker price/flow/relative-strength/momentum chart data, screenshot QA evidence, read-only portfolio analysis API/UI coverage, explicit saved-portfolio API/UI persistence coverage, read-only backtest artifact API/UI coverage, a read-only Streamlit-retirement readiness checker, and candidate service/route templates. The candidate services are not yet installed as the live production route, and there is not yet handoff-pixel A/B/C React parity or an accepted Streamlit retirement decision.
...
**Headless persistence:** `scripts/refresh_dashboard_state.py` runs the dashboard methodology without Streamlit UI, writes fresh `state.json`/transition journal data, appends the run journal, and is installed as `sector-dashboard-state-refresh.timer` after provider-flow cache warmup.
...
**Behavior:** runs on the self-hosted `sector-pi` runner, SSHes to the Pi with a pinned host key, fast-forwards the branch, syncs `requirements.txt`, runs full pytest on the Pi, enforces `MASSIVE_VERIFY_SSL = "true"` in the Pi-local Streamlit secrets without printing or changing API keys, installs/refreshes the non-sudo user timers for transition-feed exports, Massive provider snapshot capture, provider-flow cache warmup, and headless state refresh, runs a narrow secret-safe Massive/FINRA provider-flow smoke for `SPY`, warms the US-sector provider-flow cache, refreshes dashboard state/run-journal evidence headlessly, runs `scripts/check_ops_readiness.py --strict-production` so stale/missing critical production lanes fail deploy, restarts the Streamlit service by terminating its current `MainPID`, and polls `http://127.0.0.1:8501/?ticker=XLK` for HTTP `200`.
