...
**Evidence:** Next handoff browser QA screenshots and similarity reports are stored in `docs/browser-qa/next-handoff/latest`; the latest `?presentation=c` run has required text/nonblank passes for C1/C2/C3 with current similarity 0.7763 for overview, 0.8386 for deep dive, and 0.8524 for rotation. The latest `?presentation=a` terminal run has required text/nonblank passes for A1/A2/A3 with current similarity 0.8394 for overview, 0.8736 for deep dive, and 0.8939 for rotation. Baseline B profile evidence also exists against B1/B2/B3 handoff stills using the current live-data React route; required text/nonblank checks pass, while the similarity numbers document the remaining editorial visual gap: B profile 0.2784 overview, 0.2240 deep dive, 0.2479 rotation. The overview now uses cumulative stacked pillar bars and a data-backed saved-portfolio positions rail instead of handoff fixture holdings, with the QA helper preserving the current real row universe while trimming duplicate transport payload sections. The deep dive now has C2/A2-style waterfall and terminal breakdown variants, weighted pillar-detail cards with evidence tags, run-journal-backed gate/evidence panels, and collapsed advanced evidence so the first-pass visual structure remains closer to the handoff while retaining extra real-data panels on demand. The rotation screen now keeps the flow-river/lower-detail composition closer to the handoff height while using live snapshot rows for flow-pressure sorting and row-specific flow explanations.
