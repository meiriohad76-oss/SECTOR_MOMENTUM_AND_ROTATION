...
**Residual risk:** this is a foundation slice only. The API runner can execute the real headless refresh when explicitly requested, and the initial Next shell now has lockfile/audit/build evidence plus journal-backed A/B/C screen foundations, native React interactions, API-fed chart primitives, screenshot QA evidence, and candidate service/route templates. The candidate services are not yet installed as the live production route, and there is not yet handoff-pixel A/B/C React parity, chart routing, portfolio upload route, or an accepted Streamlit retirement decision.
