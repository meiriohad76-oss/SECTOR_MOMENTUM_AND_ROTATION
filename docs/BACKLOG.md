...
**Migration plan:** keep Streamlit as the production frontend until API-backed React screens reach parity. Next slices should add handoff-pixel A/B/C screen parity, chart primitives, browser screenshot QA against the handoff artifacts, API systemd/Cloudflare deployment, and only then retire the Streamlit route.
...
**Residual risk:** this is a foundation slice only. The API runner can execute the real headless refresh when explicitly requested, and the initial Next shell now has lockfile/audit/build evidence plus journal-backed A/B/C screen foundations, native React interactions, API-fed chart primitives, and screenshot QA evidence, but there is not yet a separately deployed API worker service, handoff-pixel A/B/C React parity, chart routing, portfolio upload route, or Streamlit retirement plan.
