...
**Evidence:** Next handoff browser QA screenshots and similarity report are stored in `docs/browser-qa/next-handoff/latest`; the latest `?presentation=c` run has required text/nonblank passes for C1/C2/C3 with current similarity around 0.785 for overview, 0.84 for deep dive, and 0.85 for rotation. The overview now uses cumulative stacked pillar bars and a data-backed saved-portfolio positions rail instead of handoff fixture holdings.
