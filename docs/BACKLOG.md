...
**Residual risk:** this is a foundation slice only. The refresh API currently queues/persists jobs and events; it does not yet run the real Massive/FRED refresh worker, move live data refreshes, state-machine writes, charts, portfolio uploads, or dashboard routing out of Streamlit.
