...
**Residual risk:** this is a foundation slice only. The API runner can execute the real headless refresh when explicitly requested, and the initial Next shell now has lockfile/audit/build evidence plus journal-backed A/B/C screen foundations, native React interactions, and API-fed chart primitives, but there is not yet a separately deployed API worker service, handoff-pixel A/B/C React parity, chart routing, portfolio upload route, browser screenshot QA evidence, or Streamlit retirement plan.
