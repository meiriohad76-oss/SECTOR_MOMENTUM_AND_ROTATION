...
**Evidence:** Next handoff browser QA screenshots and similarity report are stored in `docs/browser-qa/next-handoff/latest`; the latest `?presentation=c` run has required text/nonblank passes for C1/C2/C3 with current similarity around 0.789 for overview, 0.838 for deep dive, and 0.85 for rotation. The overview now uses cumulative stacked pillar bars and a data-backed saved-portfolio positions rail instead of handoff fixture holdings. The deep dive now has a C2-style waterfall, weighted pillar-detail cards with evidence tags, and run-journal-backed gate/evidence panels without drawing fixture price data.
