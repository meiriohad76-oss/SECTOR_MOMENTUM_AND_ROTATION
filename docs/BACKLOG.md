...
**Evidence:** Next handoff browser QA screenshots and similarity reports are stored in `docs/browser-qa/next-handoff/latest`; the latest `?presentation=c` run has required text/nonblank passes for C1/C2/C3 with current similarity 0.7763 for overview, 0.8386 for deep dive, and 0.8524 for rotation. The latest `?presentation=a` terminal run has required text/nonblank passes for A1/A2/A3 with current similarity 0.8394 for overview, 0.8736 for deep dive, and 0.8939 for rotation. The latest `?presentation=b` editorial run has required text/nonblank passes for B1/B2/B3 with current similarity 0.8427 for overview, 0.7955 for deep dive, and 0.8454 for rotation. The overview now uses cumulative stacked pillar bars and data-backed positions/brief rails instead of handoff fixture holdings, with the QA helper preserving the current real row universe while trimming duplicate transport payload sections. The deep dive now has C2/A2/B2-style waterfall, terminal, and editorial variants, weighted pillar-detail cards with evidence tags, run-journal-backed gate/evidence panels, and collapsed advanced evidence so the first-pass visual structure remains closer to the handoff while retaining extra real-data panels on demand. The rotation screen now keeps flow-river/terminal/editorial rotation compositions backed by live snapshot rows for flow-pressure sorting, row-specific flow explanations, and a light editorial RRG/leaderboard treatment for B3.
