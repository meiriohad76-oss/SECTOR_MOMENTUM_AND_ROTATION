...
**Files:** `src/api_contract.py`, `src/api_status.py`, `src/api_server.py`, `tests/test_api_contract.py`, `tests/test_api_status.py`, `tests/test_api_server_static.py`, `requirements.txt`, `docs/superpowers/plans/2026-06-08-b170-production-dashboard-migration.md`.
**Behavior:** adds a pure JSON status contract for dashboard health lanes and an optional FastAPI app factory with `/api/v1/health` and `/api/v1/status`. The default API status provider reads persisted state-machine storage, run-journal metadata, and Massive provider-snapshot coverage without importing Streamlit, fetching providers, recomputing indicators, or mutating live scoring state. This gives a future React/Next.js frontend a real backend status surface while the current dashboard keeps running.
