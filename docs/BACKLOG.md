...
### B-163 - 10-year walk-forward algorithm calibration backtest - IN PROGRESS
...
**Latest slice:** B-163.7 adds a fail-closed calibrated rerun gate artifact at `docs/calibration_10y_candidate_config.json`, hashes it into both backtest and calibration metadata, and surfaces it read-only in the dashboard only when hashes verify. On current data the artifact records `skipped_insufficient_history`, no selected candidate config, no final-holdout evaluation, and `live_promotion_allowed=false`. B-163.6 adds a deterministic, research-only calibration candidate search over a small score-threshold surface. Candidate selection uses calibration windows only, records validation evidence and rejection gates, writes `docs/calibration_10y_candidates.csv`, hashes it into `docs/backtest_metadata.json`, and keeps the final holdout untouched. B-163.5 made the manual backtest runner generate the baseline calibration report, summary CSV, and metadata JSON from point-in-time labels and `calibration_label_metrics()`, recording overall and per-class positive/negative momentum hit-rate evidence while keeping the dashboard read-only. Earlier slices: B-163.4 adds a dashboard Calibration Lab that surfaces the frozen baseline config, split metadata, and calibration report/summary/candidate artifacts from `docs/` without running calibration on page load; dashboard status-model tests cover `PENDING`, `UNVERIFIED`, and `VERIFIED` baseline hash states. B-163.3 adds a pure calibration-label metrics aggregator over the B-163.2 point-in-time label table; B-163.2 adds point-in-time feature/label rows from historical scored snapshots, states, target weights, and close prices without filling missing future horizons; B-163.1 freezes the current methodology baseline into deterministic config metadata, writes `docs/calibration_10y_baseline_config.json` from the manual backtest runner, records the canonical config hash in `docs/backtest_metadata.json`, and adds 10-year rolling walk-forward split helpers with no-lookahead validation and untouched final-holdout summary output. These slices do not promote any calibrated parameter or rule into live scoring.
