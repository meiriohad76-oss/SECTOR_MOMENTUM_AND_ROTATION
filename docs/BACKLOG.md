...
**Behavior:** runs on the self-hosted `sector-pi` runner, SSHes to the Pi with a pinned host key, fast-forwards the branch, syncs `requirements.txt`, runs full pytest on the Pi, installs/refreshes the non-sudo user timers for transition-feed exports and Massive provider snapshot capture, prints the secret-safe ops-readiness JSON, restarts the Streamlit service by terminating its current `MainPID`, and polls `http://127.0.0.1:8501/?ticker=XLK` for HTTP `200`.
