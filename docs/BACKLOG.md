...
### B-122 · RSS / iCal feed of state transitions — IMPLEMENTED / USER TIMER LIVE VALIDATED / PUBLIC ROUTE PENDING
**Status:** RSS and iCal transition feed generation plus optional static-public feed copies are implemented in `backlog-stepwise-qa`; Pi artifact generation, public-copy writing, and the AHADPI5 non-sudo user timer are live-validated, while external Cloudflare/feed subscription validation remains deployment configuration.
...
**Residual risk:** external feed hosting/subscription smoke is still environment dependent. AHADPI5 port `8080` currently serves a Uvicorn HTML fallback for `/feeds/...`, so HTTP `200` alone is not accepted as public feed proof; final validation requires the public route to serve RSS XML and iCal text from `public/feeds`. iCal line folding and corrupted-date hardening remain future compatibility polish.
...
**Status:** static PWA alert shell, service worker, browser subscription capture UI, local subscription registration script, high-severity notification feed writer, dry-run config diagnostics, and best-effort Web Push sender seam are implemented in `backlog-stepwise-qa`; live browser push delivery awaits VAPID keys and mobile-browser validation.
**Files:** `src/pwa_push.py`, `scripts/send_pwa_push_notifications.py`, `scripts/register_pwa_subscription.py`, `public/pwa.html`, `public/pwa-sw.js`, `public/manifest.webmanifest`, `public/notification-feed.json`, `tests/test_pwa_push.py`, `tests/test_pwa_push_script.py`, `tests/test_remaining_backlog_app_static.py`, `.gitignore`, `.dockerignore`, `.streamlit/secrets.toml.example`, `README.md`, `docs/BACKLOG.md`.
**Activation:** configure `VAPID_PRIVATE_KEY`, `VAPID_CLAIM_EMAIL`, optional `PWA_DASHBOARD_URL`, and local browser subscriptions in `data/pwa_push_subscriptions.json`. Capture a browser subscription by opening `public/pwa.html?vapid_public_key=PUBLIC_KEY`, then register copied JSON with `./.venv/bin/python scripts/register_pwa_subscription.py --label ahad-phone < subscription.json`. Validate without sending or rewriting the feed file via `./.venv/bin/python scripts/send_pwa_push_notifications.py --dry-run`, then run `./.venv/bin/python scripts/send_pwa_push_notifications.py` after dashboard state transitions are recorded.
