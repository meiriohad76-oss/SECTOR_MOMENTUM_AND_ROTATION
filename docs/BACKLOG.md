...
**Migration plan:** keep Streamlit as the production frontend until API-backed React screens reach parity. Next slices should add React shell parity for the A/B/C screens, browser screenshot QA against the handoff artifacts, and only then retire the Streamlit route.
**Residual risk:** this is a foundation slice only. The API runner can execute the real headless refresh when explicitly requested, but there is not yet a separately deployed API worker service, React frontend, chart routing, portfolio upload route, or Streamlit retirement plan.
