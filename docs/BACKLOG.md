...
**Latest slice:** B-163.2 adds a pure point-in-time calibration feature/label table from historical scored snapshots, states, target weights, and close prices. It labels positive momentum and negative/avoid outcomes across configured forward horizons with absolute return, benchmark-relative excess return, post-entry drawdown, avoided underperformance, failed follow-through, and avoided-drawdown fields while marking unavailable future horizons without filling them. Earlier slice: B-163.1 freezes the current methodology baseline into deterministic config metadata, writes `docs/calibration_10y_baseline_config.json` from the manual backtest runner, records the canonical config hash in `docs/backtest_metadata.json`, and adds 10-year rolling walk-forward split helpers with no-lookahead validation and untouched final-holdout summary output. These slices do not calibrate parameters or promote any rule into live scoring.
