...
**Residual risk:** this is a foundation slice only. The API runner can execute the real headless refresh when explicitly requested, but there is not yet a separately deployed API worker service, A/B/C React parity, chart routing, portfolio upload route, Node build/browser QA evidence, or Streamlit retirement plan.
