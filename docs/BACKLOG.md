...
**Evidence:** Next handoff browser QA screenshots and similarity reports are stored in `docs/browser-qa/next-handoff/latest`; the latest `?presentation=c` run has required text/nonblank passes for C1/C2/C3 with current similarity 0.7763 for overview, 0.8386 for deep dive, and 0.8524 for rotation. Baseline A and B profile evidence now also exists against A1/A2/A3 and B1/B2/B3 handoff stills using the current live-data React route; required text/nonblank checks pass, while the similarity numbers document the remaining visual gap: A profile 0.5425 overview, 0.6960 deep dive, 0.6044 rotation; B profile 0.2784 overview, 0.2240 deep dive, 0.2479 rotation. The overview now uses cumulative stacked pillar bars and a data-backed saved-portfolio positions rail instead of handoff fixture holdings, with the QA helper preserving the current real row universe while trimming duplicate transport payload sections. The deep dive now has a C2-style waterfall, weighted pillar-detail cards with evidence tags, run-journal-backed gate/evidence panels, and collapsed advanced evidence so the first-pass visual structure remains closer to the handoff while retaining extra real-data panels on demand. The rotation screen now keeps the flow-river/lower-detail composition closer to the handoff height while using live snapshot rows for flow-pressure sorting and row-specific flow explanations.
