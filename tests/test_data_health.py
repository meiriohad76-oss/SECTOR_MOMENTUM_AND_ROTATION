...
def test_data_health_rows_show_ohlcv_and_fred_staleness():
    ...
    fred_data = {
        ...
        "DCOILWTICO": pd.Series([112.25], index=pd.to_datetime(["2026-05-18"])),
        "STLFSI4": pd.Series([-0.74], index=pd.to_datetime(["2026-05-15"])),
    }
    ...
    assert "DCOILWTICO 8d old" in by_source["FRED macro/regime"]["detail"]
