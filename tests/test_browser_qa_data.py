...
def test_browser_qa_ohlcv_result_returns_complete_secret_free_fixture():
    ...
    assert result.data["XLK"].index.is_monotonic_increasing
    assert result.data["XLK"]["close"].notna().all()
    assert (result.data["XLK"]["close"] > 0).all()
    assert (result.data["XLK"]["high"] >= result.data["XLK"]["low"]).all()
